...
document::parser::doc_ref_result document::parser::parse(const uint8_t *buf, size_t len, bool realloc_if_needed) {
  auto code = (error_code)json_parse(buf, len, *this, realloc_if_needed);
  valid = false;
  error = UNINITIALIZED;
  return document::parser::doc_ref_result(doc, code);
}
